...
public struct PictureManager {
  ...
  mutating func obtainWallpaper(_ workingDirectory: String, at index: Int, in region: String)
    async throws
  {
  ...
  public mutating func fetchWallpapers(_ workingDirectory: String, at region: String) async throws {
    for index in -1...self.pastWallpaperRange {
      try await self.obtainWallpaper(workingDirectory, at: index, in: region)
    }
  }

  public mutating func fetchLastWallpaper(_ workingDirectory: String, at region: String)
    async throws
  {
    for index in -1...0 {
      try await self.obtainWallpaper(workingDirectory, at: index, in: region)
  ...
  public mutating func checkWallpaperExist(
    _ workingDirectory: String, on date: String, in region: String
  ) -> Bool {
  ...
  public mutating func setWallpaper(_ workingDirectory: String, on date: String, at region: String)
    throws
  {
    if self.checkWallpaperExist(workingDirectory, on: date, in: region) {
